...
| cypress#32673, #33927 | BYO model / air-gapped — cy.prompt phones home | RepairRunner: `claude`, `cmd:`, `ollama:` (local, zero egress), `openai:` (any compatible base URL incl. Modal/vLLM); one model for both tools via `goldseam.config.mjs` | qwen2.5:14b healed an id drift via ollama, full ladder, zero egress ✓ 2026-07-04; `openai:` proven end-to-end vs real OpenAI (gpt-4o-mini) ✓ 2026-07-04; self-host recipe in `selfhost/modal/` (deploy unproven); HTTP-shape unit pins ✓ |
